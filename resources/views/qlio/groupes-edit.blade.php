...
@section('content')
<div class="mb-8">
    <h1 class="text-3xl font-bold text-gray-800 mb-2">Modifier le groupe</h1>
    <p class="text-gray-600">Ajustez les droits et membres du groupe.</p>
</div>

<div class="bg-white rounded-lg shadow-md overflow-hidden">
    <div class="p-6">
        <form action="{{ route('groupes.update', $groupe->groupe_id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" id="groupe_nom" name="groupe_nom" value="{{ old('groupe_nom', $groupe->groupe_nom) }}"
                ...
                <div>
                    ...
                    <select id="groupe_type" name="groupe_type"
                        ...
                        @foreach($types as $key => $label)
                            <option value="{{ $key }}" {{ old('groupe_type', $groupe->groupe_type) == $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                    @error('groupe_type')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div class="flex items-center space-x-2">
                    <input type="checkbox" id="actif" name="actif" value="1" {{ old('actif', $groupe->actif) ? 'checked' : '' }}
                        ...
                        <div class="p-4 border border-gray-200 rounded-lg">
                            ...
                            <select name="rights[{{ $key }}]"
                                ...
                                @foreach($levels as $levelValue => $levelLabel)
                                    <option value="{{ $levelValue }}" {{ (old('rights.' . $key, $rights[$key] ?? 0) == $levelValue) ? 'selected' : '' }}>
                                        {{ $levelLabel }}
                                    </option>
                                @endforeach
                            </select>
            ...
            <div class="mt-8">
                ...
                <x-user-multi-select name="users[]" :users="$users" :selected="old('users', $groupe->users->pluck('user_id')->toArray())" />
            </div>
