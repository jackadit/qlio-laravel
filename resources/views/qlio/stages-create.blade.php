...
@section('content')
<div class="mb-8">
    <h1 class="text-3xl font-bold text-gray-800 mb-2">Ajouter un stage</h1>
    <p class="text-gray-600">Créer une nouvelle offre de stage</p>
</div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    >
                        ...
                        @foreach($etudiants as $etudiant)
                            <option value="{{ $etudiant->user_id }}" {{ old('sta_etu_id') == $etudiant->user_id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    >
                        ...
                        @foreach($societes as $societe)
                            <option value="{{ $societe->soc_id }}" {{ old('sta_soc_id') == $societe->soc_id ? 'selected' : '' }}>
                                {{ $societe->soc_nom }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    >
                        ...
                        @foreach($typesStages as $type)
                            <option value="{{ $type->typsta_id }}" {{ old('sta_typsta_id') == $type->typsta_id ? 'selected' : '' }}>
                                {{ $type->typsta_libelle }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    >
                        ...
                        @foreach($tuteurs as $tuteur)
                            <option value="{{ $tuteur->user_id }}" {{ old('sta_tutpeda_id') == $tuteur->user_id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    >
                        <option value="">-- Sélectionner un tuteur --</option>
                        @foreach($tuteurs as $tuteur)
                            <option value="{{ $tuteur->user_id }}" {{ old('sta_tutsoc_id') == $tuteur->user_id ? 'selected' : '' }}>
                                {{ $tuteur->user_nom }} {{ $tuteur->user_prenom }}
                            </option>
                        @endforeach
                    </select>
